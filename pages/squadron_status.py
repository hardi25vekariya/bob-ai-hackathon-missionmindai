...
k1.metric("AIRCRAFT",   fleet_metrics["total_assets"])
k2.metric("READY",    fleet_metrics["mission_ready_count"],
          delta=f"+{fleet_metrics['mission_ready_count']} ready")
k3.metric("GROUNDED", fleet_metrics["non_ready_count"],
          delta=f"-{fleet_metrics['non_ready_count']} grounded", delta_color="inverse")
k4.metric("READINESS",   f"{fleet_metrics['readiness_rate_pct']}%")
k5.metric("CRITICAL", critical_count,
          delta="action req" if critical_count > 0 else "none",
